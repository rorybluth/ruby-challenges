...
def give_message(number)
	...
	when 9
		puts "This is the teacher. Number Nine is a tolerant, somewhat impractial, and sympathetic vibration. Ruled by Mars."
	else
		puts "Sad Day. You have no future."
	end
end

def run
	birthdate = get_birthdate
	sumbday = sum_bday(birthdate)

	sumbday = sumbday.to_s
	number = sumbday[0].to_i + sumbday[1].to_i

	if number > 9
	number = number[0].to_i + number[1].to_i
	end

	givenumber = give_number(number)
	givemessage = give_message(number)
	puts "#{givenumber} \n#{givemessage}"
end
run
